...
//getting buttons
let buttons = new Set(menu.map(element => element.category));
...
let writeButtons = () => {
  buttons.forEach(element => {
    ...
    newButton.classList.add("btn-item", "btn", "btn-outline-dark");
    newButton.setAttribute("onclick", "writeMenu(this.innerText)")
...
function writeMenu(wantedCategory) {
  ...
  if (wantedCategory == "All") {
    menu.forEach(element => { createElement(element) })
  } else {
    let filteredMenu = menu.filter(element => element.category == wantedCategory);
    filteredMenu.forEach(element => { createElement(element) });
  }
}
